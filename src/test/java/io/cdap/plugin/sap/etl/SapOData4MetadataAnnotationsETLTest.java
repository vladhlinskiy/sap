...
public class SapOData4MetadataAnnotationsETLTest extends BaseSapODataSourceETLTest {
  ...
  @Test
  public void testNullExpressionAnnotation() throws Exception {
    StructuredRecord expected = AnnotationRecordBuilder.builder("NullAnnotated")
      .setTerm(DESCRIPTION_TERM)
      .withConstantExpression(EdmExpression.EdmExpressionType.Null, null)
      .build();

    Map<String, String> properties = sourceProperties("$select=NullAnnotated");
    List<StructuredRecord> records = getPipelineResults(properties);
    Assert.assertEquals(2, records.size());
    for (StructuredRecord actualRecord : records) {
      StructuredRecord fieldRecord = actualRecord.get("NullAnnotated");
      assertAnnotationEquals(expected, fieldRecord, "core_description");
    }
  }
