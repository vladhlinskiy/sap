...
public class ODataEntryToRecordWithMetadataTransformer extends ODataEntryToRecordTransformer {
  ...
  private StructuredRecord extractExpressionRecord(String fieldName, CsdlExpression expression, Schema schema) {
    if (expression == null) {
      return extractSingleValueExpressionRecord("Null", null, schema);
    }
    ...
    if (dynamic.isPath()) {
      String constantExpressionValue = dynamic.asPath().getValue();
      return extractSingleValueExpressionRecord("Path", constantExpressionValue, schema);
    }
    if (dynamic.isAnnotationPath()) {
      String constantExpressionValue = dynamic.asAnnotationPath().getValue();
      return extractSingleValueExpressionRecord("AnnotationPath", constantExpressionValue, schema);
    }
    if (dynamic.isLabeledElementReference()) {
      String constantExpressionValue = dynamic.asLabeledElementReference().getValue();
      return extractSingleValueExpressionRecord("LabeledElementReference", constantExpressionValue, schema);
    }
    if (dynamic.isNavigationPropertyPath()) {
      String constantExpressionValue = dynamic.asNavigationPropertyPath().getValue();
      return extractSingleValueExpressionRecord("NavigationPropertyPath", constantExpressionValue, schema);
    }
    if (dynamic.isPropertyPath()) {
      String constantExpressionValue = dynamic.asPropertyPath().getValue();
      return extractSingleValueExpressionRecord("PropertyPath", constantExpressionValue, schema);
    }
    if (dynamic.isNull()) {
      return extractSingleValueExpressionRecord("Null", null, schema);
    }
    ...
    if (logical.getType() == CsdlLogicalOrComparisonExpression.LogicalOrComparisonExpressionType.Not) {
      ...
      return extractNotExpressionRecord(fieldName, logical, value, schema);
    }
  ...
  private StructuredRecord extractNotExpressionRecord(String fieldName, CsdlExpression expression,
                                                      CsdlExpression value, Schema schema) {
    Schema leftFieldSchema = schema.getField(SapODataConstants.EXPRESSION_VALUE_FIELD_NAME).getSchema();
    return StructuredRecord.builder(schema)
      .set(SapODataConstants.EXPRESSION_NAME_FIELD_NAME, "Not") // TODO expression.getExpressionName()
  ...
  private StructuredRecord extractApplyExpressionRecord(String fieldName, CsdlApply apply, Schema schema) {
    ...
    for (CsdlExpression parameterExpression : apply.getParameters()) {
      String name = parameterExpression.toString(); // TODO
    ...
    return StructuredRecord.builder(schema)
      .set(SapODataConstants.EXPRESSION_NAME_FIELD_NAME, "Apply") // TODO getExprName
  ...
  private StructuredRecord extractCastExpressionRecord(String fieldName, CsdlCast cast, Schema schema) {
    ...
    return StructuredRecord.builder(schema)
      .set(SapODataConstants.EXPRESSION_NAME_FIELD_NAME, "Cast")  // TODO getExprName
  ...
  private StructuredRecord extractIfExpressionRecord(String fieldName, CsdlIf edmIf, Schema schema) {
    ...
    return StructuredRecord.builder(schema)
      .set(SapODataConstants.EXPRESSION_NAME_FIELD_NAME, "If")  // TODO getExprName
  ...
  private StructuredRecord extractIsOfExpressionRecord(String fieldName, CsdlIsOf isOf, Schema schema) {
    ...
    return StructuredRecord.builder(schema)
      .set(SapODataConstants.EXPRESSION_NAME_FIELD_NAME, "IsOf")  // TODO getExprName
  ...
  private StructuredRecord extractLabeledElementExpressionRecord(String fieldName, CsdlLabeledElement labeledElement,
    ...
    return StructuredRecord.builder(schema)
      .set(SapODataConstants.EXPRESSION_NAME_FIELD_NAME, "LabeledElement")  // TODO getExprName
  ...
  private StructuredRecord extractUrlRefExpressionRecord(String fieldName, CsdlUrlRef urlRef, Schema schema) {
    ...
    return StructuredRecord.builder(schema)
      .set(SapODataConstants.EXPRESSION_NAME_FIELD_NAME, "UrlRef")
      .set(SapODataConstants.EXPRESSION_VALUE_FIELD_NAME, valueRecord)
      .build();
  }
  ...
  private StructuredRecord extractRecordExpressionRecord(CsdlRecord record, Schema schema) {
    ...
    return StructuredRecord.builder(schema)
      .set(SapODataConstants.EXPRESSION_NAME_FIELD_NAME, "Record") // TODO
      .set(SapODataConstants.EXPRESSION_TYPE_FIELD_NAME, type)
